...
#include "pros/rtos.h"
#include "pxr/logger/logger.hpp"
#include "pxr/util.hpp"
#include "pxr/chassis/chassis.hpp"
#include "pxr/chassis/odom.hpp"
#include "pxr/chassis/trackingWheel.hpp"
#include "pros/rtos.hpp"

pxr::OdomSensors::OdomSensors(TrackingWheel* vertical1, TrackingWheel* vertical2, TrackingWheel* horizontal1,
...
pxr::Drivetrain::Drivetrain(pros::MotorGroup* leftMotors, pros::MotorGroup* rightMotors, float trackWidth,
...
pxr::Chassis::Chassis(Drivetrain drivetrain, ControllerSettings linearSettings, ControllerSettings angularSettings,
 ...
 */
void calibrateIMU(pxr::OdomSensors& sensors) {
    int attempt = 1;
    ...
    while (attempt <= 5) {
        ...
        pxr::infoSink()->warn("IMU failed to calibrate! Attempt #{}", attempt);
        attempt++;
    }
    // check if calibration attempts were successful
    if (attempt > 5) {
        sensors.imu = nullptr;
        pxr::infoSink()->error("IMU calibration failed, defaulting to tracking wheels / motor encoders");
    }
}

void pxr::Chassis::calibrate(bool calibrateImu) {
    ...
    if (sensors.vertical1 == nullptr)
        sensors.vertical1 = new pxr::TrackingWheel(drivetrain.leftMotors, drivetrain.wheelDiameter,
                                                      -(drivetrain.trackWidth / 2), drivetrain.rpm);
    if (sensors.vertical2 == nullptr)
        sensors.vertical2 = new pxr::TrackingWheel(drivetrain.rightMotors, drivetrain.wheelDiameter,
                                                      drivetrain.trackWidth / 2, drivetrain.rpm);
...
void pxr::Chassis::setPose(float x, float y, float theta, bool radians) {
    pxr::setPose(pxr::Pose(x, y, theta), radians);
}

void pxr::Chassis::setPose(Pose pose, bool radians) { pxr::setPose(pose, radians); }

pxr::Pose pxr::Chassis::getPose(bool radians, bool standardPos) {
    Pose pose = pxr::getPose(true);
    if (standardPos) pose.theta = M_PI_2 - pose.theta;
    if (!radians) pose.theta = radToDeg(pose.theta);
    return pose;
}

void pxr::Chassis::waitUntil(float dist) {
    // do while to give the thread time to start
    do pros::delay(10);
    while (distTraveled <= dist && distTraveled != -1);
}

void pxr::Chassis::waitUntilDone() {
    do pros::delay(10);
    while (distTraveled != -1);
}

void pxr::Chassis::requestMotionStart() {
...
void pxr::Chassis::cancelMotion() {
    this->motionRunning = false;
    pros::delay(10); // give time for motion to stop
}

void pxr::Chassis::cancelAllMotions() {
    this->motionRunning = false;
    this->motionQueued = false;
    pros::delay(10); // give time for motion to stop
}

bool pxr::Chassis::isInMotion() const { return this->motionRunning; }

void pxr::Chassis::resetLocalPosition() {
    float theta = this->getPose().theta;
    pxr::setPose(pxr::Pose(0, 0, theta), false);
}

void pxr::Chassis::setBrakeMode(pros::motor_brake_mode_e mode) {
