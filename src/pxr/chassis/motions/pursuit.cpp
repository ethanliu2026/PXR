 ...
 * @return std::vector<pxr::Pose> vector of points on the path
 */
std::vector<pxr::Pose> getData(const asset& path) {
    std::vector<pxr::Pose> robotPath;
    ...
    for (std::string line : dataLines) {
        pxr::infoSink()->debug("read raw line {}", stringToHex(line));
        ...
        if (pointInput.size() != 3) {
            pxr::infoSink()->error("Failed to read path file! Are you using the right format? Raw line: {}",
                                      stringToHex(line));
            break;
        }
        pxr::Pose pathPoint(0, 0);
        ...
        pxr::infoSink()->debug("read point {}", pathPoint);
    }
 ...
 */
int findClosest(pxr::Pose pose, std::vector<pxr::Pose> path) {
    int closestPoint;
 ...
 */
float circleIntersect(pxr::Pose p1, pxr::Pose p2, pxr::Pose pose, float lookaheadDist) {
    // calculations
 ...
 */
pxr::Pose lookaheadPoint(pxr::Pose lastLookahead, pxr::Pose pose, std::vector<pxr::Pose> path, int closest,
                            float lookaheadDist) {
    ...
    for (int i = start; i < path.size() - 1; i++) {
        pxr::Pose lastPathPose = path.at(i);
        pxr::Pose currentPathPose = path.at(i + 1);

        float t = circleIntersect(lastPathPose, currentPathPose, pose, lookaheadDist);

        if (t != -1) {
            pxr::Pose lookahead = lastPathPose.lerp(currentPathPose, t);
            lookahead.theta = i;
            return lookahead;
        }
    }
 ...
 */
float findLookaheadCurvature(pxr::Pose pose, float heading, pxr::Pose lookahead) {
    // calculate whether the robot is on the left or right side of the circle
    float side = pxr::sgn(std::sin(heading) * (lookahead.x - pose.x) - std::cos(heading) * (lookahead.y - pose.y));
...
void pxr::Chassis::follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    ...
    std::vector<pxr::Pose> pathPoints = getData(path); // get list of path points
