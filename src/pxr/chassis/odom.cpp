...
// global variables
pxr::OdomSensors odomSensors(nullptr, nullptr, nullptr, nullptr, nullptr); // the sensors to be used for odometry
pxr::Drivetrain drive(nullptr, nullptr, 0, 0, 0, 0); // the drivetrain to be used for odometry
pxr::Pose odomPose(0, 0, 0); // the pose of the robot
pxr::Pose odomSpeed(0, 0, 0); // the speed of the robot
pxr::Pose odomLocalSpeed(0, 0, 0); // the local speed of the robot
...
void pxr::setSensors(pxr::OdomSensors sensors, pxr::Drivetrain drivetrain) {
    odomSensors = sensors;
    drive = drivetrain;
}

pxr::Pose pxr::getPose(bool radians) {
    if (radians) return odomPose;
    else return pxr::Pose(odomPose.x, odomPose.y, radToDeg(odomPose.theta));
}

void pxr::setPose(pxr::Pose pose, bool radians) {
    if (radians) odomPose = pose;
    else odomPose = pxr::Pose(pose.x, pose.y, degToRad(pose.theta));
}

pxr::Pose pxr::getSpeed(bool radians) {
    if (radians) return odomSpeed;
    else return pxr::Pose(odomSpeed.x, odomSpeed.y, radToDeg(odomSpeed.theta));
}

pxr::Pose pxr::getLocalSpeed(bool radians) {
    if (radians) return odomLocalSpeed;
    else return pxr::Pose(odomLocalSpeed.x, odomLocalSpeed.y, radToDeg(odomLocalSpeed.theta));
}

pxr::Pose pxr::estimatePose(float time, bool radians) {
...
void pxr::update() {
    ...
    pxr::TrackingWheel* verticalWheel = nullptr;
    pxr::TrackingWheel* horizontalWheel = nullptr;
